    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string) => {
        const payloads: Record<string, unknown> = {
          ...
          'http://localhost:3000/admin/applications?status=pending': { items: [] }, 'http://localhost:3000/admin/reviews': { items: [] }, 'http://localhost:3000/admin/reports?status=open': { items: [], page: 1, pageSize: 50, total: 0 },
  ...
  it('approves a pending application from the applications tab', async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      const payloads: Record<string, unknown> = {
        ...
        'http://localhost:3000/admin/reviews': { items: [] }, 'http://localhost:3000/admin/reports?status=open': { items: [], page: 1, pageSize: 50, total: 0 },
        'http://localhost:3000/admin/users': { items: [] },
